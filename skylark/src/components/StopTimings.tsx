...
function StopNameListing({ stopCodes }: { stopCodes: string[] }) {
	...
	return (
		<Text weight="medium" style={{ whiteSpace: "nowrap" }}>
			{stops.map((stop) => stop.description).join(" · ") || "Route details unavailable"}
		</Text>
	);
}

export default function StopTimings({ busStopCode }: { busStopCode: string }) {
	const { services, error, isLoading, isValidating } = usePublicBusArrivals(busStopCode);

	if (isLoading) {
		return <Text color="gray" as="p" mt="4" align="center">Loading...</Text>;
	}

	if (error) {
		return <Text color="gray" as="p" mt="4" align="center">Unable to load public bus arrivals.</Text>;
	}
						...
						<Box className={styles.serviceHeader}>
							...
							<Box className={styles.serviceDetailsWrapper}>
								<Box className={styles.serviceDetails}>
									<StopNameListing
										stopCodes={service.arrivals[0]?.destinationCode ? [service.arrivals[0].destinationCode] : []}
									/>
								</Box>
							...
							<Box className={styles.serviceETA}>
								{service.arrivals.slice(0, 3).map((arrival, index) => (
									<Text key={`${arrival.estimatedArrival}-${index}`} as="span">
										{index > 0 ? "  " : ""}{formatArrival(arrival.estimatedArrival)}
									</Text>
								))}
								{isValidating && <Text className={styles.refreshIcon} as="span">↻</Text>}
							</Box>
						</Box>
					</Link>
				</Box>
